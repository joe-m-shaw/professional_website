...
We can use a binomial distribution calculation, as recommended by the Association for Molecular Pathology in their [guidelines for validation of NGS panels](https://pubmed.ncbi.nlm.nih.gov/28341590/).

I will be perfectly honest: I am not a statistician.
...
If you want to look at the full binomial distribution equation, I've included a worked example at the end.
...
The answer is 138X, and this is (sort of) why that number is seen so frequently in PanSolid documentation.
For the full story, read the Appendix.
...
## Where did 138X originally come from?

The reality is that 138X was decided as a coverage threshold in previous validations, and it is simpler to keep things the same.

However, there have been two separate justifications for 138X as a threshold.

The original way 138X was decided on was by asking what coverage was required to confidently *exclude* a variant with a true VAF of 4%.

If you start by saying that you require at least 2 reads to clinically report a variant, then a variant present in only 1 read should not be reported.

You can then calculate the VAF of 1 variant read at different coverage levels, including the 95% confidence limits, as shown in the table below.
The key row is where the upper 95% confidence threshold is just below 4%, which is at 138X.

```{r}
#| label: binom-test

data_df <- data.frame(
  total_reads = c(50, 100, 138, 150, 200, 278, 300)) |> 
  mutate(variant_reads = 1,
         vaf = round((variant_reads / total_reads)*100,2)) |> 
  rowwise() |> 
  mutate(
    conf_upper = round((binom.test(x = variant_reads,
                                   n = total_reads)$conf.int[2])*100, 2),
    conf_lower = round((binom.test(x = variant_reads,
                                   n = total_reads)$conf.int[1])*100, 2)) |> 
  rename("Total reads" = total_reads,
         "Variant reads" = variant_reads,
         "VAF (%)" = vaf,
         "VAF 95% confidence upper" = conf_upper,
         "VAF 95% confidence lower" = conf_lower)
